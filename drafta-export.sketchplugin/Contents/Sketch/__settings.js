...
module.exports = "file://" + String(context.scriptPath).split(".sketchplugin/Contents/Sketch")[0] + ".sketchplugin/Contents/Resources/_webpack_resources/75fde787bb6b7d75be0530d27f569d5b.html";
...
module.exports = "file://" + String(context.scriptPath).split(".sketchplugin/Contents/Sketch")[0] + ".sketchplugin/Contents/Resources/_webpack_resources/3c1b6ac8124d62a1a8cf1078c1217bfd.html";
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* WEBPACK VAR INJECTION */(function(fetch) {/* harmony import */ var sketch_module_web_view__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! sketch-module-web-view */ "./node_modules/sketch-module-web-view/lib/index.js");
...
var Fiber = __webpack_require__(/*! sketch/async */ "sketch/async");

var template = __webpack_require__(/*! ../resources/export.html */ "./resources/export.html");

/* harmony default export */ __webpack_exports__["default"] = (function () {
  var document = __webpack_require__(/*! sketch/dom */ "sketch/dom").getSelectedDocument();

  var username = Settings.settingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_username"));
  var token = Settings.settingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_token"));

  if (document === undefined || !document) {
    UI.message('Please open document!');
    return false;
  }

  if (username === undefined || username === '' || token === undefined || token === '') {
    Settings.setSettingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_export_after_login"), true);

    __webpack_require__(/*! ./settings */ "./src/settings.js").default(); // eslint-disable-line


    return false;
  }

  var selectedLayers = toArray(document.selectedLayers);
  var filteredLayers = selectedLayers.filter(function (item) {
  ...
  if (filteredLayers.length === 0) {
    ...
    var options = {
      identifier: "".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, ".export"),
      alwaysOnTop: true,
      width: 576,
      height: 396,
      show: false,
    ...
    browserWindow.on('closed', function () {
      if (isUploading) {
        isCanceled = true;
        UI.message('The upload has been canceled!');
      }
    });
    webContents.on('did-finish-load', function () {
      browserWindow.show();
    });
    webContents.on('requestInitialData', function () {
      var document = __webpack_require__(/*! sketch/dom */ "sketch/dom").getSelectedDocument(); // eslint-disable-line


      var username = Settings.settingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_username"));
      var token = Settings.settingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_token"));
      var selectedProject = Settings.documentSettingForKey(document, "".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_selected_project_id"));
      var selectedSize = Settings.documentSettingForKey(document, "".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_selected_size"));
      var selectedLayers = toArray(document.selectedLayers);
      var filteredLayers = selectedLayers.filter(function (item) {
        return item.type === 'Artboard';
      });
      var params = {
        ...
        exportCount: filteredLayers.length
      };
      ...
      webContents.executeJavaScript("initialData('".concat(JSON.stringify(params), "')")).catch(function () {});
    });
    webContents.on('openUrl', function (s) {
      NSWorkspace.sharedWorkspace().openURL(NSURL.URLWithString(s));
    });
    webContents.on('saveSettingsForProject', function (s) {
      var data = JSON.parse(s);

      if (data.selectedProject && data.selectedSize) {
        Settings.setDocumentSettingForKey(document, "".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_selected_project_id"), data.selectedProject);
        Settings.setDocumentSettingForKey(document, "".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY, "_selected_size"), data.selectedSize);
      }
    });
    webContents.on('upload', function (s) {
      isCanceled = false;
      isUploading = true;
      var data = JSON.parse(s);

      if (data.selectedProject && data.selectedSize && username !== undefined && username !== '' && token !== undefined && token !== '') {
        var _document = __webpack_require__(/*! sketch/dom */ "sketch/dom").getSelectedDocument();

        var _selectedLayers = toArray(_document.selectedLayers);

        var _filteredLayers = _selectedLayers.filter(function (item) {
          return item.type === 'Artboard';
        });

        var totalCount = _filteredLayers.length;
        var totalCountPercentage = _filteredLayers.length + 1;
        var statusData = {
          progress: Math.round(1 * 100 / totalCountPercentage),
          status: isCanceled
        };
        webContents.executeJavaScript("exportStatus('".concat(JSON.stringify(statusData), "')")).catch(console.error);

        var loadItems = function loadItems(uploadStack, index, results, files, callback) {
          if (isCanceled) {
            callback({
              type: 'final',
              results: results
            ...
            sketch.export(uploadStack[index], {
              output: getCacheDirectory(),
              'use-id-for-name': true,
              scales: "".concat(data.selectedSize),
              'save-for-web': true
            });
            var fullPath = "".concat(getCacheDirectory(), "/").concat(uploadStack[index].id).concat(data.selectedSize > 1 ? '@' + data.selectedSize + 'x' : '', ".png");
            var savedArtboardPath = NSURL.fileURLWithPath(fullPath);
            files.push(savedArtboardPath);
            var imageBinary = readBinaryFile(savedArtboardPath);
            var imageBase64 = imageBinary.base64EncodedStringWithOptions(0);
            var body = JSON.stringify({
              "filename": "".concat(uploadStack[index].name).concat(data.selectedSize > 1 ? '@' + data.selectedSize + 'x' : ''),
              "source": 'data:image/png;base64,' + imageBase64
            });
            fetch("".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].API_URL_HTTPS, "/projects/").concat(data.selectedProject, "/update"), {
              method: 'POST',
              headers: {
                'Accept': "application/json;version=".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].API_VERSION),
                'Content-Type': 'application/json',
                'User-Agent': "Sketch ".concat(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_VERSION),
                'X-AUTH-TOKEN': token
              },
              body: body
            }).then(function (response) {
              var statusData = {
                progress: Math.round((index + 2) * 100 / totalCountPercentage),
                status: isCanceled
              };
              webContents.executeJavaScript("exportStatus('".concat(JSON.stringify(statusData), "')")).catch(console.error);
              ...
              if (response.status === 400) {
                ...
                if (uploadStack[index]) {
                  results.push({
                    artboard: uploadStack[index].name,
                    status: response.status === 204 || response.status === 200 ? 'uploaded' : 'failed'
                  });
                }

                if (uploadStack[index + 1]) {
                  loadItems(uploadStack, index + 1, results, files, callback);
                } else {
                  callback({
                    type: 'final',
                    results: results
        ...
        var results = [];
        var files = [];
        var fiberInstance = Fiber.createFiber();
        loadItems(_filteredLayers, 0, results, files, function (state) {
          isUploading = false;
          ...
          if (state.type === 'authError') {
            webContents.executeJavaScript('authError()').catch(function () {});
          } else {
            var successfullyUploaded = state.results.filter(function (item) {
              return item.status === 'uploaded';
            }).length;
            var successData = {
              title: "".concat(successfullyUploaded, " of ").concat(totalCount, " files has been uploaded"),
              projectUrl: "https://drafta.co/".concat(data.selectedProject)
            };
            webContents.executeJavaScript("success('".concat(JSON.stringify(successData), "')")).catch(function () {});
          }

          fiberInstance.cleanup();
        });
      }
    });
    webContents.on('cancelUploading', function () {
      isCanceled = true;
    });
    webContents.on('closeBrowserWindow', function () {
      browserWindow.hide();
    });
    browserWindow.loadURL(template);
  }
});

function getCacheDirectory() {
  var fileManager = NSFileManager.defaultManager();
  var cachesURL = fileManager.URLsForDirectory_inDomains(NSCachesDirectory, NSUserDomainMask).lastObject(); // eslint-disable-line

  return cachesURL.URLByAppendingPathComponent(_config__WEBPACK_IMPORTED_MODULE_1__["default"].APP_KEY).path();
}

function deleteFileByURL(url) {
  var fileManager = NSFileManager.defaultManager();
  var error;
  var deleted = fileManager.removeItemAtURL_error(url, error); // eslint-disable-line
}

var readBinaryFile = function readBinaryFile(filepath) {
  return NSData.alloc().initWithContentsOfFile(filepath);
};

function toArray(nsArray) {
  return nsArray.map(function (el) {
    return el;
  });
}
...
/* harmony import */ var sketch_module_web_view__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(sketch_module_web_view__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _export__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./export */ "./src/export.js");
...
var template = __webpack_require__(/*! ../resources/settings.html */ "./resources/settings.html");

function settings() {
  var options = {
    identifier: "".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, ".settings"),
    alwaysOnTop: true,
    width: 576,
    height: 396,
    show: false,
  ...
  webContents.on('requestInitialData', function () {
    var username = Settings.settingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_username"));
    var token = Settings.settingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_token"));
    var params = {
      username: undefined
    };
  ...
  webContents.on('saveUserAuthSettings', function (s) {
    var data = JSON.parse(s);

    if (data.username && data.token) {
      Settings.setSettingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_username"), data.username);
      Settings.setSettingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_token"), data.token);
    }

    if (Settings.settingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_export_after_login"))) {
      Object(_export__WEBPACK_IMPORTED_MODULE_1__["default"])();
      Settings.setSettingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_export_after_login"), false);
    }
  });
  webContents.on('clearUserAuthSettings', function () {
    Settings.setSettingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_username"), '');
    Settings.setSettingForKey("".concat(_config__WEBPACK_IMPORTED_MODULE_2__["default"].APP_KEY, "_token"), '');
  });
  webContents.on('closeBrowserWindow', function () {
    browserWindow.hide();
  });
