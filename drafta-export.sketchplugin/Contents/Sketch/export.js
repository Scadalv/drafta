      ...
      if (data.selectedProject && data.selectedSize && username !== undefined && username !== '' && token !== undefined && token !== '') {
        ...
        var loadItems = function loadItems(uploadStack, index, results, files, callback) {
          if (isCanceled) {
            ...
            var fullPath = "".concat(getCacheDirectory(), "/").concat(uploadStack[index].name).concat(data.selectedSize > 1 ? '@' + data.selectedSize + 'x' : '', ".png");
...
var sketch = __webpack_require__(/*! sketch */ "sketch"); // eslint-disable-line no-unused-vars
